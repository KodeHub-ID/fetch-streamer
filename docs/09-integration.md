...
Firebase ID tokens expire after 1 hour. When a 401 is received, get a fresh token and start a new instance:

```ts
import { FetchStreamer, FetchStreamerHttpError } from '@kodehub.id/fetch-streamer';
import { auth } from '@/lib/firebase';

function connectDeviceStream(url: string): { close: () => void } {
  let instance: FetchStreamer | undefined;

  async function start(): Promise<void> {
    const token = await auth.currentUser?.getIdToken(/* forceRefresh */ true);
    if (!token) return; // currentUser signed out between retries
    instance = new FetchStreamer(url, {
      headers: { Authorization: `Bearer ${token}` },
      reconnectOnError: false,
      onMessage(event) { /* ... */ },
      onError(err) {
...
Callbacks (`onMessage`, `onError`, etc.) are proxied through a ref so they never go stale. Configuration options are snapshot at mount — add changed values to `deps` to trigger a reconnect with new config (e.g. rotated token).
